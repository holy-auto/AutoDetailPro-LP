...
-- =============================================================================
-- Idempotent — safe to re-run. Uses CREATE TABLE IF NOT EXISTS, IF NOT EXISTS
-- on indexes, and DROP/CREATE for policies.
--
-- Adds:
...
--   - boost_purchases.stripe_payment_intent_id
-- =============================================================================
...
DROP POLICY IF EXISTS "Anyone authenticated can read flags" ON feature_flags;
...
DROP POLICY IF EXISTS "Participants can view own calls" ON masked_calls;
CREATE POLICY "Participants can view own calls" ON masked_calls FOR SELECT
  USING (auth.uid() = caller_id OR auth.uid() = callee_id);
DROP POLICY IF EXISTS "Admins can manage all calls" ON masked_calls;
CREATE POLICY "Admins can manage all calls" ON masked_calls FOR ALL USING (
  EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin')
);

CREATE INDEX IF NOT EXISTS idx_masked_calls_order ON masked_calls (order_id);
CREATE INDEX IF NOT EXISTS idx_masked_calls_proxy ON masked_calls (proxy_number) WHERE status = 'active';
...
DROP POLICY IF EXISTS "Anyone authenticated can read trip stats" ON trip_stats;
CREATE POLICY "Anyone authenticated can read trip stats" ON trip_stats FOR SELECT
  USING (auth.uid() IS NOT NULL);
DROP POLICY IF EXISTS "Service role only writes" ON trip_stats;
CREATE POLICY "Service role only writes" ON trip_stats FOR ALL USING (FALSE);

CREATE INDEX IF NOT EXISTS idx_trip_stats_origin ON trip_stats (origin_lat_cell, origin_lng_cell, hour_of_day);
...
DROP POLICY IF EXISTS "Anyone authenticated can read areas" ON service_areas;
CREATE POLICY "Anyone authenticated can read areas" ON service_areas FOR SELECT
  USING (auth.uid() IS NOT NULL);
DROP POLICY IF EXISTS "Admins can manage areas" ON service_areas;
CREATE POLICY "Admins can manage areas" ON service_areas FOR ALL USING (
  EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin')
);

CREATE INDEX IF NOT EXISTS idx_service_areas_status ON service_areas (status) WHERE status IN ('active', 'soft_launch');
CREATE INDEX IF NOT EXISTS idx_service_areas_bbox ON service_areas (min_lat, max_lat, min_lng, max_lng);

-- Seed only if no rows yet
INSERT INTO service_areas (prefecture, city, min_lat, max_lat, min_lng, max_lng, status)
SELECT '東京都', NULL, 35.50, 35.85, 139.55, 139.92, 'active'
WHERE NOT EXISTS (SELECT 1 FROM service_areas WHERE prefecture = '東京都' AND city IS NULL);

INSERT INTO service_areas (prefecture, city, min_lat, max_lat, min_lng, max_lng, status)
SELECT '大阪府', NULL, 34.55, 34.85, 135.40, 135.65, 'soft_launch'
WHERE NOT EXISTS (SELECT 1 FROM service_areas WHERE prefecture = '大阪府' AND city IS NULL);
...
DROP POLICY IF EXISTS "Users insert own signals" ON fraud_signals;
CREATE POLICY "Users insert own signals" ON fraud_signals FOR INSERT
  WITH CHECK (auth.uid() = user_id);
DROP POLICY IF EXISTS "Admins read all signals" ON fraud_signals;
...
CREATE INDEX IF NOT EXISTS idx_fraud_signals_device ON fraud_signals (device_id, created_at DESC);
